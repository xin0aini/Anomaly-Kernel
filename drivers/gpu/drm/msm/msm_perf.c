...
static int wait_sample(struct msm_perf_state *perf)
{
    ...
    if (time_after(perf->next_jiffies, start_jiffies)) {
        unsigned long remaining_jiffies = perf->next_jiffies - start_jiffies;
...
static int refill_buf(struct msm_perf_state *perf)
{
    ...
    if ((perf->cnt++ % 32) == 0) {
        /* Reuse a static header buffer instead of recalculating */
        ...
        ret = msm_gpu_perfcntr_sample(gpu, &activetime, &totaltime, ARRAY_SIZE(cntrs), cntrs);
        if (ret < 0)
            return ret;

        /* Efficient calculation without frequent formatting */
        int val = totaltime ? 1000 * activetime / totaltime : 0;
        n = snprintf(ptr, rem, "%3d.%d%%", val / 10, val % 10);
        ptr += n;
        rem -= n;

        for (i = 0; i < ret; i++) {
            /* Cycle counters conversion to MHz */
            val = cntrs[i] / 10000;
            n = snprintf(ptr, rem, "\t%5d.%02d", val / 100, val % 100);
            ptr += n;
            rem -= n;
        }
    }
...
static ssize_t perf_read(struct file *file, char __user *buf, size_t sz, loff_t *ppos)
{
...
int msm_perf_debugfs_init(struct drm_minor *minor)
{
    ...
    ent = debugfs_create_file("perf", S_IFREG | S_IRUGO, minor->debugfs_root, perf, &perf_debugfs_fops);
    if (!ent) {

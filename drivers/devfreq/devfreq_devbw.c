...
static int set_bw(struct device *dev, int new_ib, int new_ab)
{
    ...
    DEV_DEBUG(dev, "BW MBps: AB: %d IB: %d\n", new_ab, new_ib);
    ...
    if (ret) {
        ...
        ret = msm_bus_scale_client_update_request(d->bus_client, i);
        if (ret)
            dev_err(dev, "Retry failed, bandwidth request still failing (%d)\n", ret);
    } else {
        d->cur_idx = i;
        d->cur_ib = new_ib;
        d->cur_ab = new_ab;
...
int devfreq_add_devbw(struct device *dev)
{
    ...
    if (of_find_property(dev->of_node, PROP_PORTS, &len)) {
        ...
        ret = of_property_read_u32_array(dev->of_node, PROP_PORTS, ports, len);
        if (ret)
            return ret;
    ...
    d->bw_data.active_only = of_property_read_bool(dev->of_node, PROP_ACTIVE);

    // Set up paths for bandwidth management
    ...
    // Devfreq profile setup
    p = &d->dp;
    p->polling_ms = 50;  // Can be dynamically adjusted later based on system load
